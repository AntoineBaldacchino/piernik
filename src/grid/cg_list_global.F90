         ...
         select type (lst => wna%lst)
            type is (na_var_4d)
               ...
               do icr= 1, nspc
                  ...
                  if (cr_spectral(icr)) then

                     do i = flind%crspcs(icr)%nbeg, flind%crspcs(icr)%nend

                        if (cr_names(icr)=='p+' .or. cr_names(icr)=='e-') then

                           write(var, '(a3,a2,a1,I2.2)') "cr_",cr_names(icr),"n", i - flind%crspcs(icr)%nbeg + 1

                        else if (cr_names(icr)=='Be10') then

                           write(var, '(a3,a4,a1,I2.2)') "cr_",cr_names(icr),"n", i - flind%crspcs(icr)%nbeg + 1

                        else

                           write(var, '(a3,a3,a1,I2.2)') "cr_",cr_names(icr),"n", i - flind%crspcs(icr)%nbeg + 1

                        endif
                        call lst(wna%fi)%set_compname(i, var)
                     enddo

                     do i = flind%crspcs(icr)%ebeg, flind%crspcs(icr)%eend

                        if (cr_names(icr)=='p+' .or. cr_names(icr)=='e-') then

                           write(var, '(a3,a2,a1,I2.2)') "cr_",cr_names(icr),"e", i - flind%crspcs(icr)%ebeg + 1

                        else if (cr_names(icr)=='Be10') then

                           write(var, '(a3,a4,a1,I2.2)') "cr_",cr_names(icr),"e", i - flind%crspcs(icr)%ebeg + 1

                        else

                           write(var, '(a3,a3,a1,I2.2)') "cr_",cr_names(icr),"e", i - flind%crspcs(icr)%ebeg + 1
